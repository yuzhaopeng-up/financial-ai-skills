...
# 表ID映射（ArkClaw已创建）
TABLE_IDS = {
    "skill_tracking": "tbltI2rJqY8RqQq",      # Skill追踪
    "article_publish": "tblsK9mNpL2vVvV",      # 文章发布
    "node_status": "tblxY3nQrZ5wWwW",          # 节点状态
    "task_board": "tblmP4oStA6xXxX",           # 任务看板
    "daily_metrics": "tblnQ5pTuB7yYyY",        # 每日指标
}
    ...
    def add_skill(self, name: str, repo_url: str, stars: int = 0, 
        ...
        return self._request("POST", f"/{table_id}/records", json={
            "fields": {
                ...
                "Forks": forks,
                "PV": pv,
                "仓库URL": {"text": repo_url, "link": repo_url},
                "发布日": datetime.now().strftime("%Y-%m-%d"),
                "负责节点": responsible,
                "状态": status,
                "备注": note
            }
    ...
    def update_node_status(self, node_name: str, status: str = "在线",
        ...
        search_result = self._request("POST", f"/{table_id}/records/search", json={
            "filter": {
                "conjunction": "and",
                "conditions": [
                    {"field_name": "节点名称", "operator": "is", "value": node_name}
                ]
            }
        })
        ...
        fields = {
            "节点名称": node_name,
            "状态": status,
            "最后活跃": last_seen or datetime.now().strftime("%Y-%m-%d %H:%M"),
            "当前任务数": tasks,
            "Token消耗": token_used,
            "备注": note
        }
    ...
    def add_task(self, task_name: str, flywheel: str = "飞轮1",
        ...
        return self._request("POST", f"/{table_id}/records", json={
            "fields": {
                ...
                "截止日期": deadline,
                "状态": status,
                "创建日": datetime.now().strftime("%Y-%m-%d")
            }
        })
...
def record_skill_push(name: str, repo_url: str, responsible: str) -> dict:
    ...
    return writer.add_skill(name, repo_url, responsible=responsible)
...
if __name__ == "__main__":
    ...
    if not FEISHU_APP_ID:
        ...
        print("✅ 连接成功")
        # 测试写入节点状态
        result = writer.update_node_status("Hermes_Brain", "在线", tasks=3)
        print(f"写入结果: {result}")
