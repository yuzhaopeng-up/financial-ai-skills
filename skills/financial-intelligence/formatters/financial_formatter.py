...
继承BaseFormatter，提供6大财务场景的IM平台Markdown格式化
"""
...
class FinancialFormatter:
    ...
    @staticmethod
    def format_change_emoji(value) -> str:
        """根据涨跌返回emoji"""
        if value is None:
            return "➖"
        if isinstance(value, (int, float)):
    ...
    def format_invoice(result: Dict[str, Any]) -> str:
        ...
        lines.append("---")
        lines.append("🤖 *Financial AI | 财务智能体*")
    ...
    def format_budget(result: Dict[str, Any]) -> str:
        ...
        lines.extend(["---", "🤖 *Financial AI | 财务智能体*"])
        return "\n".join(lines)
