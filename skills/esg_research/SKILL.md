...
- **50+ 中国上市公司**：覆盖宁德时代、比亚迪、招商银行、工商银行、中国平安、贵州茅台、阿里巴巴、腾讯、隆基绿能、长江电力等
...
# 生成文本报告
report = engine.generate_report("宁德时代")
print(report)

# 生成JSON格式
json_report = engine.generate_report("招商银行", format="json")

# 生成Markdown格式
md_report = engine.generate_report("腾讯", format="markdown")
...
# 输出到文件
python3 scripts/esg_cli.py generate "ESG查询 腾讯" --format=text --output /tmp/腾讯_esg.md
